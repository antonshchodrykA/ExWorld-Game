// Copyright 1998-2019 Epic Games, Inc. All Rights Reserved.
/*===========================================================================
	Generated code exported from UnrealHeaderTool.
	DO NOT modify this manually! Edit the corresponding .h files instead!
===========================================================================*/

#include "UObject/GeneratedCppIncludes.h"
#include "PivotTool/Private/Widgets/SPivotPreset.h"
#ifdef _MSC_VER
#pragma warning (push)
#pragma warning (disable : 4883)
#endif
PRAGMA_DISABLE_DEPRECATION_WARNINGS
void EmptyLinkFunctionForGeneratedCodeSPivotPreset() {}
// Cross Module References
	PIVOTTOOL_API UEnum* Z_Construct_UEnum_PivotTool_EPivotToolPresetUIOrientation();
	UPackage* Z_Construct_UPackage__Script_PivotTool();
	PIVOTTOOL_API UEnum* Z_Construct_UEnum_PivotTool_EPivotToolPresetType();
// End Cross Module References
	static UEnum* EPivotToolPresetUIOrientation_StaticEnum()
	{
		static UEnum* Singleton = nullptr;
		if (!Singleton)
		{
			Singleton = GetStaticEnum(Z_Construct_UEnum_PivotTool_EPivotToolPresetUIOrientation, Z_Construct_UPackage__Script_PivotTool(), TEXT("EPivotToolPresetUIOrientation"));
		}
		return Singleton;
	}
	template<> PIVOTTOOL_API UEnum* StaticEnum<EPivotToolPresetUIOrientation>()
	{
		return EPivotToolPresetUIOrientation_StaticEnum();
	}
	static FCompiledInDeferEnum Z_CompiledInDeferEnum_UEnum_EPivotToolPresetUIOrientation(EPivotToolPresetUIOrientation_StaticEnum, TEXT("/Script/PivotTool"), TEXT("EPivotToolPresetUIOrientation"), false, nullptr, nullptr);
	uint32 Get_Z_Construct_UEnum_PivotTool_EPivotToolPresetUIOrientation_Hash() { return 1595416006U; }
	UEnum* Z_Construct_UEnum_PivotTool_EPivotToolPresetUIOrientation()
	{
#if WITH_HOT_RELOAD
		UPackage* Outer = Z_Construct_UPackage__Script_PivotTool();
		static UEnum* ReturnEnum = FindExistingEnumIfHotReloadOrDynamic(Outer, TEXT("EPivotToolPresetUIOrientation"), 0, Get_Z_Construct_UEnum_PivotTool_EPivotToolPresetUIOrientation_Hash(), false);
#else
		static UEnum* ReturnEnum = nullptr;
#endif // WITH_HOT_RELOAD
		if (!ReturnEnum)
		{
			static const UE4CodeGen_Private::FEnumeratorParam Enumerators[] = {
				{ "Horizontal", (int64)Horizontal },
				{ "Vertical", (int64)Vertical },
			};
#if WITH_METADATA
			const UE4CodeGen_Private::FMetaDataPairParam Enum_MetaDataParams[] = {
				{ "Horizontal.Name", "Horizontal" },
				{ "ModuleRelativePath", "Private/Widgets/SPivotPreset.h" },
				{ "Vertical.Name", "Vertical" },
			};
#endif
			static const UE4CodeGen_Private::FEnumParams EnumParams = {
				(UObject*(*)())Z_Construct_UPackage__Script_PivotTool,
				nullptr,
				"EPivotToolPresetUIOrientation",
				"EPivotToolPresetUIOrientation",
				Enumerators,
				UE_ARRAY_COUNT(Enumerators),
				RF_Public|RF_Transient|RF_MarkAsNative,
				UE4CodeGen_Private::EDynamicType::NotDynamic,
				(uint8)UEnum::ECppForm::Regular,
				METADATA_PARAMS(Enum_MetaDataParams, UE_ARRAY_COUNT(Enum_MetaDataParams))
			};
			UE4CodeGen_Private::ConstructUEnum(ReturnEnum, EnumParams);
		}
		return ReturnEnum;
	}
	static UEnum* EPivotToolPresetType_StaticEnum()
	{
		static UEnum* Singleton = nullptr;
		if (!Singleton)
		{
			Singleton = GetStaticEnum(Z_Construct_UEnum_PivotTool_EPivotToolPresetType, Z_Construct_UPackage__Script_PivotTool(), TEXT("EPivotToolPresetType"));
		}
		return Singleton;
	}
	template<> PIVOTTOOL_API UEnum* StaticEnum<EPivotToolPresetType>()
	{
		return EPivotToolPresetType_StaticEnum();
	}
	static FCompiledInDeferEnum Z_CompiledInDeferEnum_UEnum_EPivotToolPresetType(EPivotToolPresetType_StaticEnum, TEXT("/Script/PivotTool"), TEXT("EPivotToolPresetType"), false, nullptr, nullptr);
	uint32 Get_Z_Construct_UEnum_PivotTool_EPivotToolPresetType_Hash() { return 618328028U; }
	UEnum* Z_Construct_UEnum_PivotTool_EPivotToolPresetType()
	{
#if WITH_HOT_RELOAD
		UPackage* Outer = Z_Construct_UPackage__Script_PivotTool();
		static UEnum* ReturnEnum = FindExistingEnumIfHotReloadOrDynamic(Outer, TEXT("EPivotToolPresetType"), 0, Get_Z_Construct_UEnum_PivotTool_EPivotToolPresetType_Hash(), false);
#else
		static UEnum* ReturnEnum = nullptr;
#endif // WITH_HOT_RELOAD
		if (!ReturnEnum)
		{
			static const UE4CodeGen_Private::FEnumeratorParam Enumerators[] = {
				{ "PresetType_BoundCenter", (int64)PresetType_BoundCenter },
				{ "PresetType_Center", (int64)PresetType_Center },
				{ "PresetType_Corner", (int64)PresetType_Corner },
				{ "PresetType_Edge", (int64)PresetType_Edge },
			};
#if WITH_METADATA
			const UE4CodeGen_Private::FMetaDataPairParam Enum_MetaDataParams[] = {
				{ "ModuleRelativePath", "Private/Widgets/SPivotPreset.h" },
				{ "PresetType_BoundCenter.DisplayName", "Boudnig Box Center" },
				{ "PresetType_BoundCenter.Name", "PresetType_BoundCenter" },
				{ "PresetType_Center.DisplayName", "Center" },
				{ "PresetType_Center.Name", "PresetType_Center" },
				{ "PresetType_Corner.DisplayName", "Corner" },
				{ "PresetType_Corner.Name", "PresetType_Corner" },
				{ "PresetType_Edge.DisplayName", "Edge" },
				{ "PresetType_Edge.Name", "PresetType_Edge" },
			};
#endif
			static const UE4CodeGen_Private::FEnumParams EnumParams = {
				(UObject*(*)())Z_Construct_UPackage__Script_PivotTool,
				nullptr,
				"EPivotToolPresetType",
				"EPivotToolPresetType",
				Enumerators,
				UE_ARRAY_COUNT(Enumerators),
				RF_Public|RF_Transient|RF_MarkAsNative,
				UE4CodeGen_Private::EDynamicType::NotDynamic,
				(uint8)UEnum::ECppForm::Regular,
				METADATA_PARAMS(Enum_MetaDataParams, UE_ARRAY_COUNT(Enum_MetaDataParams))
			};
			UE4CodeGen_Private::ConstructUEnum(ReturnEnum, EnumParams);
		}
		return ReturnEnum;
	}
PRAGMA_ENABLE_DEPRECATION_WARNINGS
#ifdef _MSC_VER
#pragma warning (pop)
#endif
