...
class ALSV4_CPP_API AALSBaseCharacter : public AEXWCharacter
{
	GENERATED_BODY()
...
public:
	...
	UFUNCTION(BlueprintCallable, Server, Unreliable, Category = "ALS|Character States")
	...
	UFUNCTION(BlueprintCallable, Category = "ALS|Movement System")
	void SetMovementModel(const FDataTableRowHandle& NewMovementModel);

	UFUNCTION(BlueprintCallable, BlueprintPure, Category = "ALS|Movement System")
	FDataTableRowHandle GetMovementModel() {return MovementModel;};
	...
	UFUNCTION(BlueprintCallable, Category = "ALS|Camera System")
	void SetCameraDistance(const float NewCameraDistance) { CameraDistance = NewCameraDistance; }
	...
	UFUNCTION(BlueprintCallable, Category = "ALS|Essential Information")
	void SetFootIKValueAdjustments(FDataTableRowHandle NewAdjustmentRowHandle);
...
public:
	...
	UPROPERTY(ReplicatedUsing = OnRep_IKAdjustments,EditDefaultsOnly, BlueprintReadWrite, Category = "ALS|IK")
	FALSAnimFootIKAdjustment FootIKAdjustment;

	// for  ik adjustment replication on data load
	UFUNCTION()
	void OnRep_IKAdjustments();
...
protected:
	...
	/** Updates the rotation mode depending on the velocity of the character every a period of time */
	FTimerHandle OnUpdateRotationModeTimer;
