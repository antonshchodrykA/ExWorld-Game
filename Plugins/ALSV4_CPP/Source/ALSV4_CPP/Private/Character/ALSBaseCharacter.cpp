...
void AALSBaseCharacter::Restart()
{
	...
	AEXWPlayerController* NewController = Cast<AEXWPlayerController>(GetController());
	if (NewController)
	{
...
void AALSBaseCharacter::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME(AALSBaseCharacter, FootIKAdjustment);
}
...
void AALSBaseCharacter::BeginPlay()
{
	...
	if (UWorld* World = GetWorld())
	{
		...
		World->GetTimerManager().SetTimer(OnUpdateRotationModeTimer, TimerCallback, 2.f, true, 2.f);
	}
}
...
void AALSBaseCharacter::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	...
	if (UWorld* World = GetWorld())
	{
		if(OnUpdateRotationModeTimer.IsValid())
			World->GetTimerManager().ClearTimer(OnUpdateRotationModeTimer);
...
void AALSBaseCharacter::Tick(float DeltaTime)
{
	...
	// Check Stamina for Sprinting
	if (DesiredGait == EALSGait::Sprinting)
	{
		float CurrentStamina = 100.f;
		if (GetAttributeCurrentValue(EEXWAttributeType::Stamina, CurrentStamina))
		{
			if (CurrentStamina < 3)
			{
...
void AALSBaseCharacter::RagdollStart()
{
	...
	and if the host is a dedicated server, change character mesh optimization option to avoid z-location bug*/
...
void AALSBaseCharacter::SetMovementModel(const FDataTableRowHandle& NewMovementModel)
{
...
void AALSBaseCharacter::UpdateRotationMode()
{
...
void AALSBaseCharacter::SetFootIKValueAdjustments(FDataTableRowHandle NewAdjustmentRowHandle)
{
	const FString ContextString = GetFullName();
	if (NewAdjustmentRowHandle.DataTable) {
		FALSAnimFootIKAdjustment* OutRow =
			NewAdjustmentRowHandle.DataTable->FindRow<FALSAnimFootIKAdjustment>(NewAdjustmentRowHandle.RowName, ContextString);
		if (OutRow)
		{
			FootIKAdjustment = *OutRow;
		}
		else
		{
			UE_LOG(LogTemp, Warning, TEXT("Cannot set the Foot IK value because it is not a valid row in the datatable."));
		}
	}
}
...
void AALSBaseCharacter::OnRep_IKAdjustments()
{
	UDataTable* IKDT;
	...
	FString RaceSTR = UEnum::GetDisplayValueAsText(Race).ToString().Replace(TEXT("R_"), TEXT(""));
	FString GenderStr = UEnum::GetDisplayValueAsText(Gender).ToString().Replace(TEXT("G_"), TEXT(""));

	IKParams.RowName = FName(FString(RaceSTR) + (GenderStr));
	SetFootIKValueAdjustments(IKParams);
}
